...
### Fixed
* **Workflows Ran With an Unrestricted `GITHUB_TOKEN`** - `integration-test-tls.yml` declared no `permissions` block and runs directly on `pull_request`, so it received the repository default rather than a scoped token (CodeQL `actions/missing-workflow-permissions`). It now grants `contents: read`, and `test.yml` and `coverage.yml` state the same explicitly so adding a trigger to either later cannot silently widen the token
